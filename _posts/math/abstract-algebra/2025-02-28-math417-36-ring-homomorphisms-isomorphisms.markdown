...
<!------------------------------------------------------------------------->
<h3>Example 1</h3>
Take $$R = \mathbb{Z}$$ and take $$I$$ to be a principle ideal generated by some element $$d$$ so $$I = (d) = \mathbb{Z}d$$. Then
<div>
$$
\begin{align*}
R/I = \mathbb{Z}/\mathbb{Z}d = \mathbb{Z}_d
\end{align*}
$$
</div>
<hr>
...
<!------------------------------------------------------------------------->
<h3>Example 2</h3>
Take the ring of polynomials $$R[x]$$. If we quotient this ring by the principle ideal generated by $$(x^2 + 1)$$, then we get the ring of complex numbers. $$R[x]/(x^2+1) \cong \mathbb{C}$$
<hr>
...
<!------------------------------------------------------------------------->
<h3>Example</h3>
Suppose $$K = \mathbb{Q}$$ be the rationals ring, $$S = \mathbb{Q}[x]/(f)$$ be the quotient ring and let $$f = x^2 - 2$$ so $$I = (f)$$ is the ideal generated by this polynomial $$f=x^2-2$$. 
<br>
<br>
...
$$
\begin{align*}
(a + bx) + I = a + b\bar{x} \quad a,b \in \mathbb{Q}
\end{align*}
$$
</div>
This notation is a shorthand for: given $$a \in \mathbb{Q}$$, then write $$a$$ or $$\bar{a}$$ instead of writing $$a + I \in S$$  where $$a+I$$ is the coset of constant polynomials in $$S$$. Also write $$\bar{x}$$ for the element $$x + I$$. So, then
<div>
$$
\begin{align*}
...
Remark: This quotient ring $$S$$ is isomorphic to $$\mathbb{Q}(\sqrt{2})$$ which is a subring of $$\mathbb{R}$$. $$\mathbb{Q}(\sqrt{2})$$ is the ring
<div>
$$
\begin{align*}
...
$$
\begin{align*}
I = \ker(\varphi) &= \{ f \in \mathbb{Q}[x] \ | \ \varphi(f) = 0 \} \\
 &= \{ f \in \mathbb{Q}[x] \ | \ f(\sqrt{2}) = 0 \}
\end{align*}
$$
</div>
So it's any polynomial that has $$\sqrt{2}$$ as a root. Since $$\mathbb{Q}$$ is a field, then we know that the kernel is principle. So it's generated by a single monic polynomial of minimal degree. First of all, this kernel isn't trivial. For example $$x^2 - 2$$ has a $$\sqrt{2}$$ as a root of degree 2. If we know that we don't have constant polynomials or polynomials of degree 1, then we know $$x^2 - 2$$ is a generator for the group. Constant non-zero polynomials don't have roots. Linear polynomials can't have $$\sqrt{2}$$ as a root. The roots will be rationals since the coefficients are rational.
<br>
<br>
So now we have a homomorphism $$\varphi$$ from $$\mathbb{Q}[x]$$ to $$\mathbb{R}$$. Moreover, we also have the canonical quotient homomorphism from $$\mathbb{Q}[x]$$ to $$\mathbb{Q}[x]/(x^2-2)$$.
<div>
$$
\begin{align*}
\pi: \mathbb{Q}[x] &\rightarrow \mathbb{Q}[x]/(x^2-2) \\ 
\phi(f) &= f + (x^2 - 2)
\end{align*}
$$
</div>
The homomorphism takes a polynomial $$f$$ in $$\mathbb{Q}[x]$$ to its coset.
<br>
<br>
So notice now that we have a homomorphism $$\varphi: \mathbb{Q}[x] \rightarrow \mathbb{R}$$ and we have the canonical homomorphism $$\pi: \mathbb{Q}[x] \rightarrow \mathbb{Q}[x]/(x^2-2)$$. Before we can apply the isomorphism theorem, we have one issue which is that $$\varphi$$ is not surjective. We can fix this by restricting the target to only the image of $$\varphi$$ so
<div>
$$
\begin{align*}
\varphi': \mathbb{Q}[x] &\rightarrow \varphi(\mathbb{Q}[x]) \\ 
f(x) &\rightarrow f(\sqrt{2}) \\
\end{align*}
$$
</div>
So now we have a surjective homomorphism where the kernel is $$I = (x^2 - 2)$$ and we formed the quotient ring $$\mathbb{Q}[x]/(x^2-2)$$. Therefore, we can now apply the isomorphism to conclude that $$\mathbb{Q}[x]/(x^2-2)$$ is isomorphic to $$\varphi(\mathbb{Q}[x])$$. Moreover, we have an isomorphism defined by
<div>
$$
\begin{align*}
\bar{\varphi}: \mathbb{Q}[x]/(x^2-2) &\rightarrow \varphi(\mathbb{Q}[x]) \\ 
f + (x^2 - 2) &\rightarrow \varphi(f) = f(\sqrt{2})
\end{align*}
$$
</div>
Note that $$\varphi(\mathbb{Q}[x])$$ are all the real numbers we can get by plugging in $$\sqrt{2}$$ into polynomials with rational coefficients. So
<div>
$$
\begin{align*}
f(x) &= a_0 + a_1x + ... + a_nx^n, \quad a_i \in \mathbb{Q} \\
f(\sqrt{2}) &= a_0 + a_1(\sqrt{2}) + ... + a_n(\sqrt{2})^n, \quad a_i \in \mathbb{Q}
\end{align*}
$$
</div>
...
$$
\begin{align*}
\varphi(\mathbb{Q}) = \{ a + b\sqrt{2} \ | \ a,b \in \mathbb{Q} \}
\end{align*}
$$
</div>
<hr>
...
<!------------------------------------------------------------------------->
<h3>Example 2</h3>
Suppose we have a homomorphism which is just the identity function $$id$$ from $$\mathbb{R} \rightarrow \mathbb{R}$$. Based on this create the evaluation homomorphism
<div>
$$
\begin{align*}
...
The kernel of this homomorphism is generated by $$(x^3-2)$$. The image of this homomorphism is a subset of $$\mathbb{R}$$. 
<div>
$$
\begin{align*}
\mathbb{Q}[x]/(x^3-2) \cong \varphi(\mathbb{Q}[x]) \subseteq \mathbb{R}
\end{align*}
$$
</div>
<hr>
