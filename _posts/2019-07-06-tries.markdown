...
Consider the following problem: a user is typing a string (called it $$p$$) of length $$n$$ in a search engine. As the user types, we want to display the strings that the user might have intended to type. For example, If the user types "cas", we want to display all the valid words that begin with "cas". 
<br><br>
