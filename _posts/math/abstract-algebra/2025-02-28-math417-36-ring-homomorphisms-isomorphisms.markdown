...
<!------------------------------------------------------------------------->
<h4><b>Example</b></h4>
Let $$R = \mathbf{Q}[x]$$ and $$S = \mathbf{R}$$. Since we want to build a homomorphism from a polynomial ring, then we should always think of the substitution principle. Recall that the substitution principle says that there is a unique homomorphism with the property that it take polynomials to wherever we decide to take constant polynomials too and wherever it takes $$x$$ to. So define the following homomorphism based on the rules
<div>
$$
\begin{align*}
...
<!------------------------------------------------------------------------->
<h4><b>Example</b></h4>
Now consider the map from $$\mathbf{R}[x]$$ to $$\mathbf{C}$$ but restricted to the real numbers only (so constant polynomials of degree 0 only) So $$\varphi|_{\mathbf{R} = id_{\mathbf{R}}$$ and let $$\varphi(x) = i$$. 
<div>
$$
\begin{align*}
