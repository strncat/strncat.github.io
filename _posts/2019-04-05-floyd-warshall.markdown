...
Let $$G=(V,E)$$ be a weighted graph with $$V$$ vertices and $$E$$ edges. Bellman-Ford's algorithm finds the shortest paths from a source vertex $$s$$ to all other vertices in $$V$$. Bellman-Ford is a dynamic programming algorithm. The idea is that we can find the shortest path from $$u$$ to $$v$$ using at most $$i$$ edges by using  the shortest path from $$s$$ to $$v$$ using at most $$i-1$$ edges.
