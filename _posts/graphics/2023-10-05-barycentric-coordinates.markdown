...
How do we interpolate between two points on a line segment? use the parameteric form.
...
so again the alphas are the barrycenteric weights. and this whole process is called the barrycentric interpolation.
...
So using the cross product, we can compute all these areas to derive the barycenteric coordinates.
<br>
<br>
<!------------------------------------------------------------------------------------>
...
To see an example, let's say we want to find the $z'$ of a pixel. we know the $z'$ value for each vertex of the triangle. What do we do now? We just figure out the barcenteric weights and multiply the weights by the $z'$ values of each vertex of the triangle in,
<div>
$$
\begin{align*}
