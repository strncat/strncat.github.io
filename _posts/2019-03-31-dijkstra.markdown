...
<br>
<br>
In the first iteration of Dijkstra, we extra the minimum distance node in the <i>not-sure</i> list. This node is $$s$$. We then update the distance of each neighbor based on the following,
<div center>
$$
\begin{align*}
...
After updating each neightbor we get the following the values. Also, after updating all neighbors, we mark $$s$$ as sure.
